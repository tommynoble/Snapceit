...
function determineFinalCategory(suggestedCategory, suggestedConfidence, businessCategory, userCategory) {
    ...
    // For Food & Dining, accept medium confidence
    if (suggestedCategory === 'Food & Dining' && suggestedConfidence === 'medium') {
        console.log('Using medium-confidence Food & Dining category');
        return suggestedCategory;
    }
...
exports.handler = async (event) => {
    try {
        ...
        for (const record of event.Records) {
            // Process both new receipts and updates
            if (record.eventName !== 'INSERT' && record.eventName !== 'MODIFY') continue;
            
            const newReceipt = record.dynamodb.NewImage;
            
            // Skip if already processed and not being updated
            if (newReceipt.status?.S === 'processed' && record.eventName !== 'MODIFY') {
                continue;
            }
            ...
            // Update the receipt in DynamoDB
            const updateParams = {
                TableName: process.env.RECEIPTS_TABLE,
                Key: {
                    receiptId: newReceipt.receiptId.S
                },
                UpdateExpression: 'SET suggestedCategory = :sc, categoryConfidence = :cc, category = :fc, taxDeductible = :td, #st = :st',
                ExpressionAttributeNames: {
                    '#st': 'status'
                },
                ExpressionAttributeValues: {
                    ':sc': categoryResult.category,
                    ':cc': categoryResult.confidence,
                    ':fc': finalCategory,
                    ':td': taxDeductible,
                    ':st': 'processed'
                }
        ...
        return {
            statusCode: 200,
            body: JSON.stringify('Successfully processed receipt updates')
        };
    } catch (error) {
