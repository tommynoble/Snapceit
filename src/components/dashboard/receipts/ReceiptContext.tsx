...
export const ReceiptProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const createReceipt = async (receipt: any) => {
    try {
      ...
      // Return mock receipt (actual ID will be generated by DB, but we use the generated one for optimistic UI)
      const newReceipt = {
        id: receiptId, // Use the generated UUID instead of 'pending'
        user_id: currentUser.id,
        merchant: receipt.merchant || 'Unknown Merchant',
        total: receipt.total || 0,
        status: 'pending',
        image_url: receipt.imageUrl || null, // Ensure image is passed for preview
      };
