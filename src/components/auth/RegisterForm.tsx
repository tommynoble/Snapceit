...
export function RegisterForm({ onBack, heading = "Complete your registration" }: RegisterFormProps) {
  ...
  useEffect(() => {
    ...
    // Pre-fill email from localStorage if it was set by the popup
    const popupEmail = localStorage.getItem('popupEmail');
    if (popupEmail) {
      setFormData(prev => ({
        ...prev,
        email: popupEmail
      }));
      // Clear the localStorage after using it so regular signups aren't affected
